...
Proof.
intros H' X H'0; elim H'0; auto with v62.
unfold D at 3 in |- *; auto with v62.
unfold Add in |- *.
...
2: exists maj;
    ...
    auto with v62.
intros H'2 H'6.
elim (H' bsupA x); [ intros bsup E1; elim E1 | idtac ]; auto with v62.
2: red in |- *; simpl in |- *.
2: intros H'7 H'8; exists maj; elim E; auto 7 with v62.
intros H'5 H'7; try assumption.
exists bsup; try assumption.
apply LUaux2 with (a := bsupA) (b := x); auto with v62.
Qed.
...
red in |- *; simpl in |- *; intros x H'0; elim H'0; auto with v62.
...
apply Lubs_of_finite_parts_def with (Y := Empty_set U); auto with v62.
intros x1 x2 H'4; red in H'4.
lapply (H'4 x1); [ intro H'1; elim H'1 | idtac ]; auto with v62.
intros bsup Y H'0 H'2 H'3; try assumption.
lapply (H'4 x2); [ intro H'6; elim H'6 | idtac ]; auto with v62.
...
lapply (Union_preserves_Finite U Y Y0);
 ...
 auto with v62.
elim (K (Union U Y Y0)); [ intros bsup1 E | idtac | idtac ]; auto with v62.
exists bsup1; split.
apply Lubs_of_finite_parts_def with (Y := Union U Y Y0); auto with v62.
apply Upper_Bound_Couple_intro.
apply Lub_is_in_Carrier with (X := Y); auto with v62.
apply Lub_is_in_Carrier with (X := Y0); auto with v62.
apply Lub_is_in_Carrier with (X := Union U Y Y0); auto with v62.
elim H'3.
...
intros H'4 H'5; apply Lub_definition; auto with v62.
apply Upper_Bound_definition; auto with v62.
apply Lub_is_in_Carrier with (X := Lubs_of_finite_parts X); auto with v62.
red in |- *; simpl in |- *; intros x H'8; elim H'8.
intros bsup0 Y H'9 H'10 H'11; apply Lub_is_in_Carrier with Y; auto with v62.
intros y H'8; elim H'4.
intros H'9 H'10; apply H'10.
apply Lubs_of_finite_parts_def with (Y := Singleton U y); auto with v62.
red in |- *; simpl in |- *.
intros x H'11; elim H'11; auto with v62.
apply Singleton_is_finite.
intros y H'8; apply H'5.
apply Upper_Bound_definition.
elim H'8; auto with v62.
intros y0 H'9; elim H'9.
intros bsup1 Y H'10 H'11 H'12; elim H'12.
intros H'13 H'14; apply H'14.
apply Upper_downward_stable with (B := X); auto with v62.
Qed.
...
elim (H'7 (Lubs_of_finite_parts X)); [ intros bsup0 E0 | apply LFP_directed ];
 auto with v62.
exists bsup0; auto with v62.
auto with v62.
intros Y H'4 H'5; apply Pairs_are_enough_finite_case; auto with v62.
