...
unfold D at 6 in |- *; auto with v62.
clear H'0 X.
...
generalize H'7.
rewrite (Empty_set_zero' U x).
unfold Add at 1 in |- *; auto with v62.
clear H'5 H'3 H'1 H'0.
cut
 (forall X : Ensemble U,
  Included U X (Add U (Add U (Add U A x) x0) x1) ->
  Included U X (Carrier_of U D)); auto with v62.
intro L1.
cut
 (forall X : Ensemble U,
...
2: apply
    ...
    auto with v62.
intro L2.
cut (Included U (Add U A x) (Add U (Add U (Add U A x) x0) x1)); auto with v62.
intro F1.
...
clear H'0.
apply LUaux2 with (a := bsup2) (b := bsup1); auto with v62.
...
red in |- *; simpl in |- *.
intros x2 H'; elim H'; auto with v62.
intros x3 H'0; elim H'0; auto with v62.
apply Lub_is_in_Carrier with (X := Add U A x); auto with v62.
apply Lub_is_in_Carrier with (X := Add U A x0); auto with v62.
intros x3 H'0; elim H'0; auto with v62.
apply Lub_is_in_Carrier with (X := Add U A x1); auto with v62.
2: apply Lub_is_in_Carrier with (X := Add U A x1); auto with v62.
apply LUaux with (A := Add U A x) (B := Add U A x0); auto with v62.
cut (Add U (Add U A x) x0 = Union U (Add U A x) (Add U A x0)).
intro H'0; rewrite <- H'0; auto with v62.
...
apply Add_distributes; auto with v62.
apply LUaux with (A := Add U A x) (B := Add U A x0); auto with v62.
rewrite <- (Add_distributes U A A x x0); auto with v62.
cut
 (Union U (Add U (Add U A x) x0) (Singleton U x1) =
  Add U (Add U (Add U A x) x0) x1 :>Ensemble U).
intro H'0; rewrite H'0.
rewrite (Add_distributes U (Add U A x) A x0 x1); auto with v62.
unfold Add at 3 in |- *; auto with v62.
Qed.
...
elim (H'7 (Lubs_of_finite_parts U C X));
 [ intros bsup0 E0 | apply LFP_directed ]; auto with v62.
exists bsup0; unfold D at 1 in |- *; auto with v62.
intros Y H'1 H'2; try assumption.
change (ex (fun bsup : U => Lub U D Y bsup)) in |- *.
apply Triples_are_enough_finite_case; auto with v62.
apply Consistent_downward_stable with (B := X); auto with v62.
Qed.
